...
import { createPaymentCheckout } from "@/features/payments/api/payments-api"
...
function CheckoutContent() {
  ...
  const transferCode = receipt?.transferContent ?? `PAY LUMIS ${planId.toUpperCase()}`
  const bankName = receipt?.bankName ?? "Vietcombank (VCB)"
  const accountNumber = receipt?.accountNumber ?? "1023 4567 890"
  const accountHolder = receipt?.accountHolder ?? "LUMIS RESEARCH PLATFORM"
  const qrCodeUrl = receipt?.qrCodeUrl ?? receipt?.qrUrl
  ...
  async function handleCreateCheckout() {
    try {
      ...
      setErrorMessage(error instanceof Error ? error.message : "Không thể tạo phiếu thanh toán.")
    } finally {
      setIsCreating(false)
    }
  }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-7">
          <div className="bg-white rounded-xl border border-[#c2c6d6]/30 p-6 shadow-sm">
              ...
              <div className="w-full md:w-[220px] shrink-0 flex flex-col items-center">
                <div className="w-[200px] h-[200px] bg-[#f8f9ff] border-2 border-dashed border-[#004191]/30 rounded-xl flex flex-col items-center justify-center mb-3">
                  {qrCodeUrl ? (
                    ...
                    <img src={qrCodeUrl} alt="Payment QR code" className="h-full w-full rounded-xl object-contain" />
                  ) : (
                    <>
                      <span className="material-symbols-outlined text-[48px] text-[#004191]/40 mb-2">qr_code_2</span>
                      <span className="text-[12px] text-[#424753] font-medium text-center px-4">Create receipt<br/>to show QR</span>
                    </>
                  )}
                </div>
              ...
              <div className="flex-1 w-full space-y-4">
                ...
                <div>
                  ...
                  <div className="font-medium text-[15px] text-[#121c2a] bg-[#f8f9ff] p-3 rounded-lg border border-[#c2c6d6]/30 flex justify-between items-center group cursor-pointer hover:border-[#004191]/50 transition-colors">
                    <span className="tracking-widest">{accountNumber}</span>
            ...
            {/* Actions */}
            {errorMessage ? (
              <div className="mt-6 rounded-lg border border-red-200 bg-red-50 p-3 text-[13px] font-semibold text-red-700">
                {errorMessage}
              </div>
            ) : null}
            <div className="mt-8 pt-6 border-t border-[#c2c6d6]/30 flex flex-col sm:flex-row gap-3">
              <button
                type="button"
                disabled={isCreating || Boolean(receipt)}
                onClick={() => void handleCreateCheckout()}
                className="flex-1 bg-gradient-to-br from-[#004191] to-[#0051d6] text-white font-semibold text-[14px] py-3 rounded-lg hover:opacity-90 transition-opacity shadow-md flex items-center justify-center gap-2 disabled:opacity-60"
              >
                <span className="material-symbols-outlined text-[18px]">{receipt ? "check_circle" : "receipt_long"}</span>
                {receipt ? "Receipt created" : isCreating ? "Creating..." : "Create payment receipt"}
              </button>
